...
test('estadisticasDe: cuenta fechas, campeonatos, podios y mejor puesto', () => {
  const stats = estadisticasDe([
    { torneo: 'A', puesto: '1', deck: '' },
    { torneo: 'B', puesto: '5', deck: '' },
    { torneo: 'C', puesto: '3', deck: '' },
  ]);
  assert.deepEqual(stats, { fechas: 3, campeonatos: 1, podios: 2, mejorPuesto: 1 });
});

test('estadisticasDe: historial vacío', () => {
  assert.deepEqual(estadisticasDe([]), { fechas: 0, campeonatos: 0, podios: 0, mejorPuesto: null });
});
